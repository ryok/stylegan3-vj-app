...
# out_local_dir = "features/"
out_dir = "features/"

def extract_feature(bpm, path):

    # out_dir = os.path.join(save_dir, out_local_dir)
    ...
    sf.write(out_dir + "lpf_y.wav", lpf_y, sr)
    lpf_y, sr = librosa.load(out_dir + "lpf_y.wav")

    lpf_slice_track = librosa.util.frame(
        lpf_y, frame_length=int(frame_length), hop_length=int(frame_length)
    )
    lpf_y_rmse = librosa.feature.rms(
        lpf_y, frame_length=int(frame_length), hop_length=int(frame_length), center=True
    )
    ...
    sf.write(out_dir + "bpf_y.wav", bpf_y, sr)
    bpf_y, sr = librosa.load(out_dir + "bpf_y.wav")

    bpf_slice_track = librosa.util.frame(
        bpf_y, frame_length=int(frame_length), hop_length=int(frame_length)
    )
    bpf_y_rmse = librosa.feature.rms(
        bpf_y, frame_length=int(frame_length), hop_length=int(frame_length), center=True
    )
    ...
    sf.write(out_dir + "hpf_y.wav", hpf_y, sr)
    hpf_y, sr = librosa.load(out_dir + "hpf_y.wav")

    hpf_slice_track = librosa.util.frame(
        hpf_y, frame_length=int(frame_length), hop_length=int(frame_length)
    )
    hpf_y_rmse = librosa.feature.rms(
        hpf_y, frame_length=int(frame_length), hop_length=int(frame_length), center=True
    )
